import 'package:jacked/src/db/db.dart';
import 'package:jacked/src/db/models/workout.dart';
import 'package:sqflite/sqflite.dart';

class WorkoutRepository {
  WorkoutRepository._({required this.db});

  static WorkoutRepository? _instance;

  final table = 'Workouts';
  Database db;

  static Future<WorkoutRepository> get instance async {
    if (_instance != null) return _instance!;
    final db = await JackedDb.database;
    _instance = WorkoutRepository._(db: db);
    return _instance!;
  }

  // Reset the singleton. Use only in tests.
  static void resetForTests() {
    _instance = null;
  }

  Future<List<Workout>> list({String? orderBy}) async {
    if (orderBy == null) {
      return (await db.query(table)).map(Workout.fromMap).toList();
    }
    return (await db.query(table, orderBy: orderBy)).map(Workout.fromMap).toList();
  }

  Future<bool> delete(int id) async => await db.delete(table, where: 'id = ?', whereArgs: [id]) > 0;

  Future<Workout?> get(int id) async {
    final maps = await db.query(table, where: 'id = ?', whereArgs: [id]);
    return maps.isNotEmpty ? Workout.fromMap(maps.first) : null;
  }

  Future<int> create(NewWorkout item) async =>
      db.insert(table, item.toMap(), conflictAlgorithm: ConflictAlgorithm.ignore);

  Future<bool> update(Workout item) async =>
      await db.update(
        table,
        item.toMap(),
        where: 'id = ?',
        whereArgs: [item.id],
      ) >
      0;
}
